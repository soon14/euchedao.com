 ...
 function getLineContent2($row)
{
    ...
    {
        $content = getJieShao_smore($row['lineid']); //行程内容
    }
    return $content;
...
function getPiclistArr($row)
{
    ...
    if(empty($row['linepic'])&&empty($row['piclist']))//没有任何图片时处理
	{
		  ...
		  $biglist =' <li><img src="'.$temp_big.'" width="611" height="auto"/></li>';
	...
	else if(empty($row['piclist'])&&!empty($row['linepic'])) //只上传封面的情况.
	{
		  ...
		  $biglist =' <li><img src="'.$biglist.'" width="600" height="auto"/></li>';
	...
	else
	{
	   ...
	   {  
		  if(empty($value))
		  {
		   ...
		   $biglist .=' <li><img src="'.$bigpic.'" alt="'.$temparr[1].'" width="611" height="auto"/></li>';
